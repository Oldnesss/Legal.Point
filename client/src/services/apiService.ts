...
class ApiService {
  ...
  static async deleteFirm(firmId: FirmType['id']): Promise<FirmType['id']> {
    try {
      ...
      if (response.status !== 200) return await Promise.reject(new Error('Failed to deleted firm'));
      return firmId;
    } catch (error) {
  ...
  static async editFirm(formData: PostFirmFormDataType, firmId: FirmType['id']): Promise<FirmType> {
    try {
      const response = await apiInstance.patch<FirmType>(`api/form/${firmId}`, formData);
